...
async def generate_comment(genres):
    ...
    if api_availability['gemini']:
        ...
        try:
            async with asyncio.timeout(15):
                model = genai.GenerativeModel('gemini-1.5-flash')
                prompt = """ یک تحلیل جامع و جذاب به زبان فارسی برای یک فیلم، با لحنی حرفه‌ای و سینمایی بنویس. نقاط قوت و ضعف فیلم را با ارائه مثال‌های مشخص از صحنه‌ها یا عناصر فیلم شرح بده. اطمینان حاصل کن که هر جمله به طور کامل بیان شده و تحلیل با یک جمع‌بندی نهایی و روشن که دیدگاه کلی درباره فیلم را منتقل می‌کند، به پایان برسد. متن باید دقیقاً بین 80 تا 120 کلمه باشد."""
                response = await model.generate_content_async(prompt)
