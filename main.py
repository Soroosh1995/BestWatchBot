...
async def generate_comment(genres):
    ...
    if api_availability['gemini']:
        ...
        try:
            async with asyncio.timeout(15):
                model = genai.GenerativeModel('gemini-1.5-flash')
                prompt = """ یک تحلیل جامع و جذاب به زبان فارسی برای یک فیلم، با لحنی حرفه‌ای و سینمایی بنویس. در این تحلیل، به تفصیل درباره جزئیات بصری، کارگردانی، بازیگری، و موسیقی متن صحبت کن و تأثیر هر یک را بر تجربه کلی فیلم شرح بده. نقاط قوت و ضعف فیلم را با ارائه مثال‌های مشخص از صحنه‌ها یا عناصر فیلم توضیح بده. مطمئن شو که هر جمله کامل باشد و تحلیل با یک جمع‌بندی نهایی و روشن به پایان برسد. متن باید دقیقاً بین 80 تا 120 کلمه باشد."""
                response = await model.generate_content_async(prompt)
            ...
            async with asyncio.timeout(15):
                ...
                data = {
                    ...
                    "messages": [
                        ...
                        {"role": "user", "content": """
یک تحلیل جامع و جذاب به فارسی برای یک فیلم بنویس، بدون ذکر نام فیلم، در 3 تا 5 جمله کوتاه و کامل (هر جمله با نقطه پایان یابد). 
لحن حرفه‌ای و سینمایی داشته باشد و متن متنوع و متفاوت از تحلیل‌های قبلی باشد. 
