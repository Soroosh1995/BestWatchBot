...
async def generate_comment(genres):
    ...
    # پرامپت جدید
    prompt = """
یک تحلیل کوتاه و جذاب به زبان فارسی درباره یک فیلم بنویس که نظر شخصی یک تماشاگر مشتاق را منتقل کند. تحلیل باید بر جنبه‌های داستانی، احساسی و الهام‌بخش فیلم تمرکز کند و به ویژگی‌های کلیدی آن (مانند خط داستانی، عمق شخصیت‌ها، سبک کارگردانی یا مضمون اصلی) به شکلی مختصر و گیرا اشاره کند. لحن باید مثبت، پرشور و متقاعدکننده باشد تا خواننده را به دیدن فیلم ترغیب کند. از زبان ساده و روان استفاده کن. اطمینان حاصل کن که متن دقیقاً بین 50 تا 80 کلمه و شامل 4 تا 6 جمله کوتاه و کامل باشد و جمله آخر، دیدگاه کلی و مثبتی را به طور کامل بیان کند. از ذکر نام فیلم یا بازیگر و تشبیهات مفصل خودداری کن.
"""
